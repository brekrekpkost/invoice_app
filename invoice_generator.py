...
LOGO_PATH = "logo.png"
PHONE = "+61404746265"
SENDER_ADDRESS = "17 Ullapool St, Heathwood"

def generate_invoice_from_db(customer, invoice_num, date, line_items, output_path,
                            sender_email=None, sender_abn=None, include_logo=True, bank=None, include_address=False):
    ...
    config = {
        ...
        "sender": {
            ...
            "phone": PHONE,
            "email": sender_email,
            "address": SENDER_ADDRESS if include_address else None
        },
        "meta": {
            "number": invoice_num,
            "date": date
...
def draw_invoice(filename, config):
    ...
    if sender.get('address'):
        c.drawString(left_margin, y, sender['address'])
        y -= 0.5 * cm
