...
def create_invoice_ui():
    ...
    # Include sender address?
    include_address = Confirm.ask("Include address (17 Ullapool St, Heathwood)?", default=False)
    ...
    customer_folder = os.path.join("invoices", customer[1])
    os.makedirs(customer_folder, exist_ok=True)
    pdf_path = os.path.join(customer_folder, f"Invoice_{invoice_num}.pdf")
    invoice_generator.generate_invoice_from_db(
        customer, invoice_num, date, line_items, pdf_path,
        sender_email, sender_abn, include_logo, bank=selected_bank, include_address=include_address
    )
...
def mark_paid_ui():
    ...
    if new_status == "paid":
        ...
        if inv:
            ...
            console.print("\n[bold]Receipt sender:[/bold]")
            console.print("1. Krepko (finance@krepko.com.au, ABN 20693126171)")
            console.print("2. Agajan Babayev (agajanbabayev11@outlook.com, ABN 12 339 140 269)")
            sender_choice = Prompt.ask("Choice", choices=["1", "2"], default="1")
            if sender_choice == "1":
                receipt_email = "finance@krepko.com.au"
                receipt_abn = "20693126171"
            else:
                receipt_email = "agajanbabayev11@outlook.com"
                receipt_abn = "12 339 140 269"

            customer_folder = os.path.join("invoices", customer[1])
            os.makedirs(customer_folder, exist_ok=True)
            receipt_path = os.path.join(customer_folder, f"Receipt_{inv[2]}.pdf")

            bank_info = {'name': inv[7], 'bsb': inv[8], 'acc': inv[9]}

            invoice_generator.generate_receipt(
                customer, inv[2], inv[3], line_items, receipt_path,
                sender_email=receipt_email, sender_abn=receipt_abn,
